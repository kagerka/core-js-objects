  ...
  objects.map((el) => {
    ...
    for (let i = 0; i < keys.length; i += 1) {
      if (!map.has(keys[i])) {
        ...
        map.set(keys[i], map.get(keys[i]) + values[i]);
      }
    }
    return map;
  });
...
function removeProperties(obj, keys) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    Object.entries(tempObj).map(([key]) => {
      if (key === keys[i]) {
        delete tempObj[key];
      }
      return tempObj;
...
function makeWord(lettersObject) {
  let result = [];
  const obj = Object.entries(lettersObject);
  for (let i = 0; i < obj.length; i += 1) {
    result.push(obj[i][1].join(''));
  }
  result = result.join('').split('');
  for (let i = 0; i < obj.length; i += 1) {
    for (let j = 0; j < obj[i][1].length; j += 1) {
      result.splice(obj[i][1][j], 1, obj[i][0]);
    }
  }
...
function group(array, keySelector, valueSelector) {
  ...
  array.map((el) => {
    if (!map.has(keySelector(el))) {
      ...
      map.set(
        keySelector(el),
        `${map.get(keySelector(el))},${valueSelector(el)}`.split(',')
      );
    }
    return map;
  });
